...
public class RatingController {
	...
	public ResponseEntity<?> delete(@PathVariable("id") Long id) {
		...
		return service.getById(id).map(rating -> {
			...
			return new ResponseEntity<>("Comentario deletado!",HttpStatus.ACCEPTED);
	...
	public ResponseEntity<?> upadate(@PathVariable("id") Long id, @RequestBody RatingDTO dto) {
		return service.getById(id).map(findRating -> {
			try {
				...
				Rating rating = new Rating(findRating.getId(), dto.getText(), findRating.getDate(), dto.getGrade(),
						findRating.getIdMovie(), user);
